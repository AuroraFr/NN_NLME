...
def main(steps=25000):
    ...
    for step in range(steps):
        ...
        if loss_value < best_loss:
            ...
            eqx.tree_serialise_leaves(model_folder+"Antibody_real_test.eqx", model)
        else:
            patience = patience - 1

        end = time.time()
        print(f"Step: {step}, Loss: {loss_value}, KL: {kl_loss}, Computation time: {end - start}")
        ...
        if (not full_stop_criteria) and patience == 0:
            ...
            if patience == 0 or (grad_norm <= eps_grad_norm and ratio <= eps_delta_norm):
                print('reached convergence', step, best_loss, grad_norm, ratio)
                break
