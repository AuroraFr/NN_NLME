...
def loss(model, y, times, state, key_i, n_z, normalized=False):
        ...
        print("after model", X.shape, targets.shape)
        const = 2. * jnp.pi
        ...
        print('reconstruction error', all_reconstruct_error.shape)
...
def main(i, steps=5000):
    ...
    measurements = 5
    T = 10
    model_folder = "EXPs/PKPD_exp/convergence_irregular_"+str(measurements)+"p_100_10d_noise"
    ...
    if not irregular:
        ...
        train_data = np.load('PKPD_datasets/'+str(measurements)+'_irregular_1latent_theta1/dataset_1.npy', allow_pickle=True)
    ...
    init_params_list = np.loadtxt('PKPD_datasets/pkpd_init_values.txt')

    init_params = init_params_list[i,:]
    init_params[2] = np.log(init_params[2])
    init_params = jnp.append(init_params, jnp.log(0.3**2))
    print(init_params)
