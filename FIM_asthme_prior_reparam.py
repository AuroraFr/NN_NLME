...
def variance_estimation(
    all_datas, model_params, N_subjects, times,
    hessian=True, sandwich=True,
    b_sample_size=2000,
    eps_ridge=1e-8
):
    ...
    for i in range(N_subjects):
        ...
        grad_log_likelihood_Y_b = jax.jacfwd(
            jax.vmap(log_likelihood, in_axes=(None, 0, None)),
            argnums=0
        )(jnp.array(model_params), epsilons, all_datas[i, :])   # (S,P)
        ...
        if hessian:
            hess_log_likelihood_Y_b = jax.jacfwd(
                ...
                argnums=0
            )(phi, epsilons, all_datas[i, :])  # (S,P,P)
