...
def generate_data(n_sample, T, timepoints, irregular=True):
    ...
    if timepoints == 10:
        # times= np.array([0, 20, 45, 85, 130, 160, 200, 260, 310, 350, 400])
        times= np.array([0, 20, 45, 85, 130, 160, 200, 260, 320, 400])
        # times= np.array([[0, 29, 32, 37, 45, 60, 249, 252, 260, 400]])
    elif timepoints == 15:
    ...
    F2_random_values = np.array(fold_change_2 * np.exp(F2_random_values))
    datas = []
    data_nonoise = []
    
    #every individual
    for Ab_production_init_acc_value, individual_F2 in zip(Ab_production_init_acc_values, F2_random_values):
...
for i in range(100):
    ...
    if irregular:
        ...
        dataset_folder = 'antibody_datasets/antibody_irregular_3dose/10_'+str(duration)+'_'+str(subjects)+'_2latent_theta_F2_2/'
        ...
        dataset_folder = 'antibody_datasets/antibody_3dose/'+str(measurements)+'_'+str(duration)+'_'+str(subjects)+'_2latent_theta_F2/'
        Path(dataset_folder).mkdir(parents=True, exist_ok=True)
        np.save(dataset_folder+'dataset_'+str(i), dataset)
        np.save(dataset_folder+'dataset_nonoise_'+str(i), dataset_nonoise)
